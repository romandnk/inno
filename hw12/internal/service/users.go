...
func SetUserBlockStatus(id primitive.ObjectID, isBlocked bool) error {
	user, err := users.GetUser(id)
	if err != nil {
	...
	if user.IsBlocked == isBlocked {
		if isBlocked {
			...
			return ErrUserAlreadyUnblocked
		}
	}
...
func SetUserRole(id primitive.ObjectID, role string) error {
	user, err := users.GetUser(id)
	if err != nil {
		return ErrUserNotFound
	}
	...
	if user.Role == role {
		return fmt.Errorf("%w: %s", ErrRoleAlreadySet, role)
	}
...
func ResetUserPassword(id primitive.ObjectID) (string, error) {
	const userPasswordLen int = 10
	...
	newPassword := hash(random.GenerateRandomString(userPasswordLen))

	user.Password = hash(newPassword)

	return newPassword, users.SetUser(user)
}
